...
DOMAIN="jenkins.boubamahir.com"
CONFIG_FILE="/etc/nginx/sites-available/$DOMAIN"

# Create Nginx configuration file
sudo tee "$CONFIG_FILE" > /dev/null <<'EOL'
upstream jenkins{
    server jenkins.boubamahir.com:8080;
}

server{
    listen 443 ssl;
    server_name jenkins.boubamahir.com;
...
## sudo ln -s "/etc/nginx/sites-available/jenkins.boubamahir.com" "/etc/nginx/sites-enabled/"
